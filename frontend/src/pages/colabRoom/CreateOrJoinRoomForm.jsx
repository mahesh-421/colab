...
import useCreateColab from '../../hooks/connections/colabRoom/useCreateColab';
import useJoinColab from '../../hooks/connections/colabRoom/useJoinColab';
...
import RecomendedUse from '../../components/colab/RecomendedUse';
...
const CreateOrJoinRoomForm = () => {
  ...
  const createcolab = useCreateColab();
  const joincolab = useJoinColab();
  ...
  const handleJoinRoom = async () => {
    ...
    await joincolab({ roomId });
    dispatch(changeActiveConnection());
    navigate(`/colabsetup/room/${roomId}`);
  };

  const handleCreateRoom = async () => {
    const roomId = uuidv4();
    await createcolab({ roomId });
    dispatch(changeActiveConnection());
    navigate(`/colabsetup/room/${roomId}`);
  };
